...
foreach sample in all eu neighbor other {
	foreach var of varlist $outcomes_simple {
		ppmlhdfe `var' p ln_good_total ln_distw ln_gdp* $dummy_vars if $`sample', noabsorb cluster($index_vars)
...
foreach sample in all eu neighbor other {
		foreach var of varlist $outcomes_simple {
			ppmlhdfe `var' p ln_good_total ln_distw ln_gdp* $dummy_vars ln_agree ln_dem_diff if $`sample', noabsorb cluster($index_vars)
